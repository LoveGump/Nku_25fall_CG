...
namespace RayCast_GroundShading::Intersection
{
    ...
    // tMin, tMax: 相交距离范围
        ...
        Vec3 n1 = t.vertexNormals[0];
        Vec3 n2 = t.vertexNormals[1];
        Vec3 n3 = t.vertexNormals[2];
        bool hasVtxNormals = (glm::length(n1) > eps) && (glm::length(n2) > eps) && (glm::length(n3) > eps);
        Vec3 shadingNormal = faceNormal;
        if (hasVtxNormals) {
            Vec3 interp = bu * n1 + bv * n2 + bw * n3;
            float nlen = glm::length(interp);
            if (nlen > eps) shadingNormal = interp / nlen;
        }
    
        // 生成命中记录，并填充 Gouraud 所需数据
        auto rec = getHitRecord(tHit, ray.at(tHit), shadingNormal, t.material);
        if (rec) {
            rec->bary = Vec3{bu, bv, bw};
            rec->vtx[0] = v1; rec->vtx[1] = v2; rec->vtx[2] = v3;
            rec->vtxNormals[0] = n1; rec->vtxNormals[1] = n2; rec->vtxNormals[2] = n3;
        }
        return rec;
    }
