...
typedef void (*ImGuiDemoMarkerCallback0)(const char *file, int line, const char *section, void *user_data);
extern ImGuiDemoMarkerCallback0 GImGuiDemoMarkerCallback0;
extern void *GImGuiDemoMarkerCallbackUserData0;
ImGuiDemoMarkerCallback0 GImGuiDemoMarkerCallback0 = NULL;
void *GImGuiDemoMarkerCallbackUserData0 = NULL;
#define IMGUI_DEMO_MARKER(section)                                                               \
  do                                                                                             \
  {                                                                                              \
    if (GImGuiDemoMarkerCallback0 != NULL)                                                       \
      GImGuiDemoMarkerCallback0(__FILE__, __LINE__, section, GImGuiDemoMarkerCallbackUserData0); \
  } while (0)

std::vector<SolverBase *> solvers{
    dynamic_cast<SolverBase *>(new LoopSubdivision::LoopSolver()),
    dynamic_cast<SolverBase *>(new root3Subdivision::Root3Solver()),
    dynamic_cast<SolverBase *>(new MeshOptimize::meshOptimSolver())};
...
{
  float transScale = 0.3f;
  eye[2] += transScale * yoffset;
  lookat[2] += transScale * yoffset;
}
...
int main(int argc, char **argv)
{
  ...
  int startIndex = gDrawObjects.size();

  auto convertObj = [&](SolverBase *solver) -> void
  {
    std::cout << "enter function\n";
    if (solver)
    {
      solver->run(inattrib, inshapes);
    }

#ifndef NDEBUG
    std::cout << "===info===\n";
    std::cout << "vertices:\n";
    for (int i = 0; i < inattrib.vertices.size(); i += 3)
    {
      std::cout << inattrib.vertices[i] << " " << inattrib.vertices[i + 1] << " " << inattrib.vertices[i + 2] << std::endl;
    }
    std::cout << "normals:\n";
    for (int i = 0; i < inattrib.normals.size(); i += 3)
      std::cout << inattrib.normals[i] << " " << inattrib.normals[i + 1] << " " << inattrib.normals[i + 2] << std::endl;
    std::cout << "shapes:\n";
    for (const tinyobj::shape_t &shape : inshapes)
    {
      std::cout << shape.name << std::endl;
      int i = 0;
      for (const tinyobj::index_t &index : shape.mesh.indices)
      {
        std::cout << index.vertex_index + 1 << " " << index.texcoord_index + 1 << " " << index.normal_index + 1 << "   ";
        if (++i % 3 == 0)
          std::cout << std::endl;
      }
      for (const unsigned int &id : shape.mesh.smoothing_group_ids)
        std::cout << id << " ";
      std::cout << std::endl;
    }
    std::cout << "======\n";
  ...
  auto drawCall = [&]() -> void
  {
    ...
    bool regen_all_normals = false;

    while (gDrawObjects.size() - startIndex < shapes.size())
    {
      gDrawObjects.emplace_back();
      glGenBuffers(1, &gDrawObjects.back().vb_id);
    }
    for (int i = startIndex + shapes.size() - 1; i < gDrawObjects.size(); ++i)
    {
      glDeleteBuffers(1, &gDrawObjects[i].vb_id);
    }

    {
      for (size_t s = 0; s < shapes.size(); s++)
      {
        DrawObject &o = gDrawObjects[startIndex + s];
  ...
  ImFont *font1 = io.Fonts->AddFontFromFileTTF("imgui/fonts/MonaspaceNeon.ttf", 20.0f);
  ...
  const char *solverNames[100];
  static int curSolverIndex = -1;
  static int oldSolverIndex = -1;
  static bool refresh = false;
  static bool firstRun = true;
  static bool running = false;
  static int showParamConfig = false;
  static char intBuf[100][32];
  static char fltBuf[100][32];
  static float resize = 0.5;
  std::thread algThread;
  SolverBase *curSolver = nullptr;
  std::vector<FltParam> fltParams;
  std::vector<IntParam> intParams;
  std::vector<std::vector<float>> numFs(solvers.size());
  std::vector<std::vector<float>> numVs(solvers.size());
  ...
  for (int i = 0; i < solvers.size(); ++i)
    solverNames[i] = solvers[i]->getName();

  // read model files
  static int modelNums = 0;
  static int curModelIndex = -1;
  const char *modelPaths[100];
  std::string folderPath = "../source/model";
  for (const auto &entry : std::filesystem::directory_iterator(folderPath))
  {
    int len = strlen(entry.path().c_str());
    if (memcmp(entry.path().c_str() + (len - 4), ".obj", 4))
      continue;
    char *path = new char[100];
    strcpy(path, entry.path().c_str());
    modelPaths[modelNums++] = path;
  }
  // base_dir = GetBaseDir(folderPath);
  base_dir = folderPath;
  if (base_dir.empty())
  {
    base_dir = ".";
  ...
  while (glfwWindowShouldClose(window) == GL_FALSE)
  {
    ...
    // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
    {
      ...
      IMGUI_DEMO_MARKER("Algorithms");
      if (ImGui::CollapsingHeader("Alogrithms"))
      {
        std::string text = "Choose Algorithm:\n";
        if (curSolverIndex != oldSolverIndex)
        {
          refresh = true;
          oldSolverIndex = curSolverIndex;
        }
        if (curSolverIndex >= 0 && curSolverIndex < modelNums)
        {
          text += curSolver->getName();
        }
        else
        {
          text += "None";
        }
        ImGui::ListBox(text.c_str(), &curSolverIndex, solverNames, solvers.size(), 5);

        // show params config page(if any)
        if (curSolverIndex >= 0 && curSolverIndex < solvers.size())
        {
          curSolver = solvers[curSolverIndex];
          fltParams = curSolver->getFltParamList();
          intParams = curSolver->getIntParamList();
          if (!fltParams.empty() || !intParams.empty())
        ...
        if (running)
        {
          if (solvers[curSolverIndex]->finMtx.try_lock())
          {
            if (solvers[curSolverIndex]->hasFinished && solvers[curSolverIndex]->sourceMtx.try_lock())
            {
              drawCall();
              algThread.join();
              running = false;
              solvers[curSolverIndex]->sourceMtx.unlock();
              std::cout << "+++algorithm(" << solvers[curSolverIndex]->getName() << ") complete+++\n";
            }
            solvers[curSolverIndex]->finMtx.unlock();
          }
          if (solvers[curSolverIndex]->updateMtx.try_lock())
          {
            if (solvers[curSolverIndex]->update && solvers[curSolverIndex]->sourceMtx.try_lock())
            {
              drawCall();
              solvers[curSolverIndex]->sourceMtx.unlock();
              solvers[curSolverIndex]->update = false;
            }
            solvers[curSolverIndex]->updateMtx.unlock();
          }
        }
        // run button
        if (ImGui::Button("Run"))
        {
          if (curSolverIndex >= 0 && curSolverIndex < solvers.size())
          {
            if (!algThread.joinable())
            {
              solvers[curSolverIndex]->hasFinished = false;
              algThread = std::thread(convertObj, solvers[curSolverIndex]);
              running = true;
            }
          }
          ...
          if (record)
          {
            ...
            for (auto shape : inshapes)
              numCur += shape.mesh.indices.size() / 3;
            numFs[curSolverIndex].push_back(log(numCur));
            // std::cout << "numcur: " << numCur << "log: " << log(numCur) << "back: " << numFs[curSolverIndex].back() << "\n";
            numVs[curSolverIndex].push_back(log(inattrib.vertices.size()));
          }
        }
        // record button
        if (!record)
      ...
      {
        std::string text = "Choose Model:\n";
        if (curModelIndex >= 0 && curModelIndex < modelNums)
        {
          text += modelPaths[curModelIndex];
        }
        else
        {
          text += "None";
        ...
        ImGui::ListBox(text.c_str(), &curModelIndex, modelPaths, modelNums, 15);

        if (ImGui::Button("Load/Reload"))
        {
          tinyobj::LoadObj(&inattrib, &inshapes, &materials, &warn, &err, modelPaths[curModelIndex], base_dir.c_str());
          if (!warn.empty())
          {
          ...
          std::thread t(convertObj, nullptr);
          t.join();
          drawCall();
        ...
        long long numCur0 = 0;
        for (auto shape : inshapes)
          numCur0 += shape.mesh.indices.size() / 3;
        std::ostringstream fsInfo, vtxInfo;
        fsInfo << "number of faces: " << numCur0;
        vtxInfo << "number of vertices: " << inattrib.vertices.size() / 3;
        ImGui::Text(fsInfo.str().c_str());
        ImGui::Text(vtxInfo.str().c_str());
        ImGui::SliderFloat("Resize", &resize, 0.0f, 4.0f);
      }
    ...
    if (showGraph)
    {
      ImGui::Begin("Graph", &showGraph);
      if (ImPlot::BeginPlot("Num of Faces", ImVec2(IMPLOT_AUTO, 650)))
      {
        ImPlot::SetupAxes("step", "ln(numFs)");
        for (int i = 0; i < solvers.size(); ++i)
        {
          if (numFs[i].empty())
            continue;
          std::vector<float> x;
          for (int j = 0; j < numFs[i].size(); ++j)
            x.push_back(j);
          ImPlot::PlotLine(solvers[i]->getName(), (float *)&x[0], (float *)&numFs[i][0], numFs[i].size());
        }
        ImPlot::EndPlot();
      }
      if (ImPlot::BeginPlot("Num of Vertices", ImVec2(IMPLOT_AUTO, 650)))
      {
        ImPlot::SetupAxes("step", "ln(numVtx)");
        for (int i = 0; i < solvers.size(); ++i)
        {
          if (numVs[i].empty())
            continue;
          std::vector<float> x;
          for (int j = 0; j < numVs[i].size(); ++j)
            x.push_back(j);
          ImPlot::PlotLine(solvers[i]->getName(), (float *)&x[0], (float *)&numVs[i][0], numVs[i].size());
        }
        ImPlot::EndPlot();
      }
    ...
    if (showParamConfig)
    {
      ...
      if (intParams.size())
      {
        ImGui::Text("int parameter:");
        for (int i = 0; i < intParams.size(); ++i)
        {
          std::ostringstream o;
          o << curSolver->getIntParam(i);
          if (refresh)
            strcpy(intBuf[i], std::string(o.str()).c_str());
          ImGui::InputText(intParams[i].name, intBuf[i], 32, ImGuiInputTextFlags_CharsDecimal);
        }
      }
      if (fltParams.size())
      {
        ImGui::Text("float parameter");
        for (int i = 0; i < fltParams.size(); ++i)
        {
          std::ostringstream o;
          o << curSolver->getFltParam(i);
          if (refresh)
            strcpy(fltBuf[i], std::string(o.str()).c_str());
          ImGui::InputText(fltParams[i].name, fltBuf[i], 32, ImGuiInputTextFlags_CharsDecimal);
        }
      }
      if (refresh)
        refresh = false;
      if (ImGui::Button("Set"))
      {
        for (int i = 0; i < intParams.size(); ++i)
        {
          int val = std::stoi(std::string(intBuf[i]));
          curSolver->setIntParam(i, val);
        }
        for (int i = 0; i < fltParams.size(); ++i)
        {
          float val = std::stof(std::string(fltBuf[i]));
          std::cout << "set: " << val << "\n";
          curSolver->setFltParam(i, val);
        }
      }
      ImGui::End();
    }
    ...
    // Fit to -1, 1
    glScalef(resize / maxExtent, resize / maxExtent, resize / maxExtent);
    ...
    if (saveImage)
    {
      std::string solverName = "original";
      if (curSolverIndex >= 0 && curSolverIndex < solvers.size())
      {
        solverName = solvers[curSolverIndex]->getName();
      }
      std::string filename = "screenshot_" + solverName + "_" + std::to_string(screenshotCount++) + ".png";
      SaveScreenshot(window, filename.c_str());
      saveImage = false;
    }
