...
namespace NRenderer
{
    ...
    struct Mesh : public Entity
    {
        ...
        vector<Vec2> uvs;             // UV坐标列表 纹理坐标
        vector<Index> normalIndices;   // 法线索引 法线索引用于定义三角形的顶点顺序
        vector<Index> positionIndices; // 顶点索引 顶点索引用于定义三角形的顶点顺序
        vector<Index> uvIndices;       // UV索引 UV索引用于定义三角形的顶点顺序
