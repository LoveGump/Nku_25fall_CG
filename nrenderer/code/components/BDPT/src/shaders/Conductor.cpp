...
Scattered Conductor::shade(const Ray &ray, const Vec3 &hitPoint, const Vec3 &normal) const
{
    ...
    if (glm::dot(ray.direction, normal) > 0)
    {
        ...
        cosine = ni_over_nt * glm::dot(ray.direction, normal) / glm::length(ray.direction);
    }
    else
    {
        // 入射角小于0，说明在物体外部
...
Vec3 Conductor::evaluate(const Vec3 &wo, const Vec3 &wi, const Vec3 &normal) const
{
...
bool Conductor::refract(const Vec3 &v, const Vec3 &n, float ni_over_nt, Vec3 &refracted) const
{
...
float Conductor::schlick(float cosine, float refractiveIndex) const
{
