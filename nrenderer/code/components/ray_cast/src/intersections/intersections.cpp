...
{
    // 光线与三角形的相交Möller–Trumbore算法计算光线与三角形的相交
    // ray: 光线
    // t: 三角形
    // tMin, tMax: 相交距离范围
    HitRecord xTriangle(const Ray& ray, const Triangle& t, float tMin, float tMax) {
        ...
        const auto& v2 = t.v2;
        const auto& v3 = t.v3;
        const auto& normal = glm::normalize(t.normal); // 归一化三角形法线
        // 计算三角形的两个边向量
        auto e1 = v2 - v1;
        auto e2 = v3 - v1;
        // 计算叉积P
    ...
    HitRecord xSphere(const Ray& ray, const Sphere& s, float tMin, float tMax) {
        ...
        // 计算二次方程系数
        Vec3 oc = ray.origin - position;    // 光线起点到球心的向量
        float a = glm::dot(ray.direction, ray.direction);// 光线方向的平方长度
        float b = glm::dot(oc, ray.direction);// 光线起点到球心向量与光线方向的点积，代表
        float c = glm::dot(oc, oc) - r*r; // 球体半径的平方
        // 计算判别式
