...
namespace Ray_KD_tracing
{
    ...
    namespace Intersection
    {
        ...
        // tMin, tMax: 相交距离范围
        // 返回相交记录
        HitRecord xMesh(const Ray &ray, const Mesh &m, float tMin = 0.f, float tMax = FLOAT_INF);

        // 光线与三角形列表相交测试（使用 KDTree 加速）
        HitRecord xTriangleBuffer(const Ray &ray, const std::vector<Triangle> &tris, float tMin = 0.f, float tMax = FLOAT_INF);
    }
}
