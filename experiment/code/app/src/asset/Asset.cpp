...
namespace NRenderer
{
    ...
    void Asset::updateNodeGlDrawData(NodeItem& node) {
        ...
        else if (np.type == T::MESH) {  // 更新网格数据
            auto& m = *meshes[np.entity];
            // 更新顶点数据
            glBindBuffer(GL_ARRAY_BUFFER, node.glVBO);
            glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(Vec3)*m.positions.size(), &m.positions[0]);
            // 更新索引数据
            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, node.glEBO);
            glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, sizeof(unsigned int)* m.positionIndices.size(), &m.positionIndices[0]);
            glBindBuffer(GL_ARRAY_BUFFER, 0);
        }
    }
