...
namespace RayCast
{
    ...
    RGB Phong::shade(const Vec3& in, const Vec3& out, const Vec3& normal) const {
        ...
        auto diffuse = diffuseColor * glm::max(0.0f, glm::dot(out, normal)); // 不能为负值，如果为负则为0
        // 计算镜面反射项：specularColor * (cos(alpha))^n
        // auto specular = specularColor * fabs(glm::pow(glm::dot(v, r), specularEx));
        auto specular = specularColor * glm::pow(glm::max(0.0f, glm::dot(v, r)), specularEx); // 也不能为负值
        // 计算环境光项：ambientColor
