...
#undef Types
        ...
        Property(const string &key, Wrapper::IntType intWrapper)
            : key(key), type(Type::INT), valueWrapper(intWrapper)
        {
        }
        Property(const string &key, Wrapper::FloatType floatWrapper)
            : key(key), type(Type::FLOAT), valueWrapper(floatWrapper)
        {
        }
        Property(const string &key, Wrapper::RGBType rgbWrapper)
            : key(key), type(Type::RGB), valueWrapper(rgbWrapper)
        {
        }
        Property(const string &key, Wrapper::RGBAType rgbaWrapper)
            : key(key), type(Type::RGBA), valueWrapper(rgbaWrapper)
        {
        }
        Property(const string &key, Wrapper::Vec3Type vec3Wrapper)
            : key(key), type(Type::VEC3), valueWrapper(vec3Wrapper)
        {
        }
        Property(const string &key, Wrapper::Vec4Type vec4Wrapper)
            : key(key), type(Type::VEC4), valueWrapper(vec4Wrapper)
        {
        }
        Property(const string &key, Wrapper::TextureIdType texIdWrapper)
            : key(key), type(Type::TEXTURE_ID), valueWrapper(texIdWrapper)
        {
        }
    };
    ...
    public:
        ...
        // 检查是否存在指定键名的属性
        inline bool hasProperty(const string &key)
        {
