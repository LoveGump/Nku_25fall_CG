    ...
    void PhotoMappingRenderer::pthotoMap(Ray &ray, Vec3 r, int currDepth)
    {
        ...
        else
        {
            ...
            if (scene.materials[mtlHandle.index()].hasProperty("diffuseColor"))
            {
                ...
                photons.push_back(p);
                if (currDepth >= depth || rand() % 1000 < 200)
                    return;
    ...
    HitRecord PhotoMappingRenderer::closestHitObject(const Ray &r)
    {
        ...
        for (auto &n : result)
        {
            for (auto &b : n->Box_list)
            {
                HitRecord hitRecord;
                if (b->type == SPHERE)
                    hitRecord = Intersection::xSphere(r, *(b->sp), 0.01, closest);
                else if (b->type == TRIANGLE)
                    hitRecord = Intersection::xTriangle(r, *(b->tr), 0.01, closest);
                if (hitRecord && hitRecord->t < closest)
                {
                    closest = hitRecord->t;
                    closestHit = hitRecord;
        ...
        if (hitObject && hitObject->t < t)
        {
            ...
            if (scene.materials[mtlHandle.index()].hasProperty("ior"))
            {
                ...
                else
                {
                    ...
                    if (scattered.refraction_ray.direction != Vec3(0.f))
                        refraction = trace(scattered.refraction_ray, currDepth + 1, gather);
            ...
            for (int i = 0; i < k_nearst.size(); i++)
            {
                ...
                if (n_dot_in <= 0.f)
                    continue;
                dir += -(p.in.direction);
                r += p.r / (float)(PI * std::pow(max_r, 2) * photonCount);
            }
