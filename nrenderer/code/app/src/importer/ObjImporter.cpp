...
namespace NRenderer
{
    ...
    bool ObjImporter::import(Asset& asset, const string& path) {
        ...
        while (getline(file, currLine)) {
            ...
            if (token == "mtllib") {  // 材质库文件
                ...
                for (int i=0; i<3; i++) {
                    ...
                    // v是顶点序号
                    // 处理顶点位置
                    // 将在mesh中的顶点存储到positions中，并记录索引到positionIndices中
                    if (v != -1) {
                        // 处理顶点位置pMap:v -> index
                        auto finded = pMap->find(v); // 查找是否已存在该顶点位置
                        if (finded != pMap->end()) {
                            vpi[i] = finded->second; // 已存在，使用已有索引
                        }
                        else {
                            // 不存在，添加新顶点位置
        ...
        // 导入成功后：为新添加的 mesh 计算顶点法线（如果缺失），然后生成OpenGL预览缓冲区
        if (successFlag) {
            // 为新增的 meshes 计算顶点法线（若没有顶点法线且有顶点/索引数据）
            for (auto mi = beginMsh; mi < asset.meshes.size(); ++mi) {
                auto& m = *asset.meshes[mi];
