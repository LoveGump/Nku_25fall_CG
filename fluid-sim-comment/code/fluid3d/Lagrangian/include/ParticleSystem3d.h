...
namespace FluidSimulation
{
    namespace Lagrangian3d
    {
        ...
        // 3D旋转木板障碍物结构体
        struct RotatingBoard3d
        {
            bool enabled = false;
            glm::vec3 center = glm::vec3(0.0f);      // 旋转中心
            glm::vec3 size = glm::vec3(0.0f);        // 木板尺寸 (长, 宽, 厚)
            glm::vec3 rotationAxis = glm::vec3(0.0f, 1.0f, 0.0f);  // 旋转轴 (默认Y轴)
            float angularVelocity = 0.0f;            // 角速度
            float angle = 0.0f;                       // 当前角度
            float restitution = 0.3f;                // 恢复系数
        ...
        public:
            ...
            // 旋转木板相关
            void setRotatingBoard(glm::vec3 center, glm::vec3 size, glm::vec3 axis, 
                                  float angularVelocity, float restitution, float friction);
            void disableRotatingBoard();
            void updateRotatingBoard(float dt);
