...
namespace Ray_KD_tracing::Intersection
{
    ...
    {
        // 使用静态缓存的 KDTree 加速网格相交测试（基于 Mesh）
        static std::unordered_map<const Mesh *, std::shared_ptr<KDTree>> kdCache;
        auto it = kdCache.find(&m);
        if (it == kdCache.end())
        {
            auto tree = std::make_shared<KDTree>();
            tree->buildFromMesh(m);
            kdCache[&m] = tree;
            it = kdCache.find(&m);
        }
        return it->second->intersect(ray, tMin, tMax);
    }

    // 光线与三角形列表的相交测试（使用 KDTree 加速）
    HitRecord xTriangleBuffer(const Ray &ray, const std::vector<Triangle> &tris, float tMin, float tMax)
    {
        static std::unordered_map<const std::vector<Triangle> *, std::shared_ptr<KDTree>> triCache;
        auto key = &tris;
        auto it = triCache.find(key);
        if (it == triCache.end())
        {
            auto tree = std::make_shared<KDTree>();
            tree->buildFromTriangles(tris);
            triCache[key] = tree;
            it = triCache.find(key);
        }
        return it->second->intersect(ray, tMin, tMax);
    }
}